...
- As a user, I want to easily identify information related to the exoplanet discovery: when was it discovered, how (detection method), where (from space or from earth), from what entity it was discovered - and also to access some of their features such as their orbital period and other information related to their planetary system.
...
See below the first wireframe I drew before starting my project. It was very general and is far from the final result but really helped me staying focused. 
...
- The modal form enables the user to refine the sample by adding filters on various criteria.
...
- The navigation bar contains (from left to right):
    ...
    * The "Reset Filters" button enables the user to reset filters for the entire dashboard as well as setting back the default parametres for the modal form.
...
- Here is a list of what information are given: 
    ...
    * the last update of the sample, 
    * the average stellar age, 
...
- discovery facility - pie chart: it provides the user with the breakdown of planets (in %) by observatory. As the number of observatory facilities is quite significant, the chart only shows the top 8 entities to improve readibility (the rest are flagged under "others"). 
...
- number of planets in system - line graph: this graph shows the number of planets for which we have identified (or not) other planet(s) belonging to the same planetary system. 
...
- This section is composed of three scatter plots that provide the user with two-dimensional representation between certain metrics of the planets and its stellar. This enables the user to establish (or not) correlations between some know metrics.
- For each scatter plot, the user has the possibility to identify planets that were (or not) discovered during the Kepler mission thanks to a colour scale.
...
- The following metrics are represented:
   * Correlation 1: mass vs radius of the planet, with the mass of the planet reprensented on the x-axis and the radius on the y-axis. Please note that this graph only pertains to planet features. The domain of the x-axis is set using the max and min values but the y-axis was set manually after analysing the data to avoid outliers effects. This increases user readibility and is a process often used in statistical analysis.
   * Correlation 2: planet mass vs. stellar mass, with the planet mass reprensented on the x-axis and the stellar mass on the y-axis. As it was done for the previous correlation, the x-axis domain was set using max and min values but the y-axis was set manually to avoir outliers effet. 
   * Correlation 3: planet radius vs. stellar radius, with the planet radius reprensented on the x-axis and the stellar radius on the y-axis. x-axis and y-axis were set manually to avoid outliers effect.  
...
- I would like to integrate the NASA API so that the user can update the sample with the latest records available, and choose specific column fields for the dataset. 
...
- I also would like to add markers/indications to provide the user with a benchmark regarding the data that are being displayed (Example: display the age of the sun in the stellar age graph...). 
...
- I used the following JS libraries combined with some core JS to create data-visualization elements:
    ...
    * [DC.js](https://dc-js.github.io/dc.js/) is used in combination of crossfilter to leverage on d3 library and add interactivity to the charts.
...
-	For this section you should be able to identify 4 charts:
    ...
    - Chart 4 : 
        - For non-cumulative data: you should be able to view a stacked bar chart with the number of detections per year. A breakdown by detection method should also be visible and the colours of the method should be consistent with the previous chart.
...
- For this section you should be able to identify 4 charts:
    - Chart 1: you should be able to view a bar chart for orbital period. For each interval period, you should view the total number of planets. By clicking the "?" icon next to the chart title, a small paragraph providing a simple definition of the orbital period should appear under the title. Clicking back the icon back should make the paragraph disappear. 
    - Chart 2: you should be able to view a line chart providing the number of planets associated to each planetary system (ranging from 1 discovered planet in the system to up to 8 planets discovered for an associated planetary system). By pointing on the line chart, you should be able to have the total number of planets apearing in a text box. 
...
- For each scatter plot, there should be:
    ...
    - when pointing at a specific plot, there should be a text box specifying the two dimensions for this specific planet (example: Planet Mass = 400 - Stellar Mass = 2)
...
5. Then, I waited just a minute and was able to access the deployed version of my project following this URL [https://alexiadelorme.github.io/project-exoplanets/](https://alexiadelorme.github.io/project-exoplanets/).
